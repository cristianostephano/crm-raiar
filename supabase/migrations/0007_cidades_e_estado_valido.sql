-- Phase 9 Plan 1: cidades (tabela de referência IBGE, read-only) + RPC
-- cidades_por_estado(p_uf) + constraint chk_estado_valido em clientes.estado
-- (LOC-01/LOC-02/LOC-04).
--
-- NEW file, never an edit a 0001-0006 (Pitfall 11 — once pushed, any further
-- change is a NEW migration file).
--
-- Source: .planning/phases/09-filtros-de-estado-e-cidade-estruturados/09-01-PLAN.md
--         .planning/phases/09-filtros-de-estado-e-cidade-estruturados/09-RESEARCH.md
--           (Pattern 1 tabela cidades read-only + RPC; Pattern 2 seed IBGE
--           offline; Pattern 3 constraint NOT VALID; Pitfall 1)
--         .planning/phases/09-filtros-de-estado-e-cidade-estruturados/09-PATTERNS.md
--         supabase/migrations/0002_clientes_and_funil.sql (historico's
--           read-only-no-insert-policy posture; chk_ganho_somente_etapa_final/
--           chk_perdido_exige_motivo NOT VALID-style naming)
--         supabase/migrations/0003_dashboard_aggregates.sql (SECURITY
--           INVOKER by omission rule, stated explicitly in its own header)

-- ─────────────────────────────────────────────────────────────────────────
-- 1. cidades — tabela de referência, seedada uma única vez com a lista
--    oficial de municípios do IBGE. NUNCA derivada de clientes.cidade
--    (perpetuaria inconsistência e bloquearia cadastrar cliente em cidade
--    nova — Anti-Pattern 7 do ARCHITECTURE.md do marco).
-- ─────────────────────────────────────────────────────────────────────────
...
-- UMA policy só: SELECT liberado para authenticated. NENHUMA policy de
-- insert/update/delete — a tabela é seed-only, mesma postura de `historico`
-- (0002_clientes_and_funil.sql: "deliberately NO user insert policy"); o
-- default-deny do Postgres para RLS já bloqueia toda escrita via API.
...
-- ─────────────────────────────────────────────────────────────────────────
-- 2. cidades_por_estado — RPC que alimenta o seletor de Cidade em cascata.
--    Deliberately NOT security definer — SECURITY INVOKER por omissão,
--    mesma regra que 0003_dashboard_aggregates.sql declara explicitamente
--    para toda function deste projeto. Aqui é defesa em profundidade (a
--    RLS de `cidades` já é read-open a todo authenticated), não um
--    requisito funcional, mas mantém a convenção consistente.
-- ─────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────
-- 3. Backfill de clientes.estado (LOC-04, D-01) — normalização simples,
--    sem fuzzy match, porque os clientes atuais são todos dados de teste
--    que o dono vai apagar de qualquer forma. Roda ANTES da constraint
--    (Pitfall 1 / chk_estado_valido not valid abaixo).
-- ─────────────────────────────────────────────────────────────────────────

-- Passo 1: normaliza espaços e caixa (ex: " sp " -> "SP", "rj" -> "RJ").
update clientes set estado = upper(trim(estado));

-- Passo 2: mapeia nome do estado por extenso (com e sem acento) -> sigla,
-- para os registros que ainda não batem com nenhuma das 27 siglas depois
-- do passo 1. Correspondência exata/case-insensitive apenas (D-01) — sem
-- fuzzy match.
update clientes
...
-- ─────────────────────────────────────────────────────────────────────────
-- 4. chk_estado_valido — adicionada NOT VALID (Pitfall 1): não escaneia/
--    bloqueia synchronously contra os dados já existentes; é enforced para
--    toda gravação NOVA a partir de agora. Naming/estilo igual a
--    chk_ganho_somente_etapa_final/chk_perdido_exige_motivo (0002).
-- ─────────────────────────────────────────────────────────────────────────
...
--    único em servicodados.ibge.gov.br/api/v1/localidades/municipios,
--    NUNCA chamado em runtime pelo app). ~5.571 municípios, quebrado em
--    lotes de até 1000 linhas por INSERT para legibilidade (sem exigência
--    funcional de fazer isso).
-- ─────────────────────────────────────────────────────────────────────────
