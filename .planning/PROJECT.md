...
- ✓ Supervisor importa clientes ativos em massa via nova planilha "Importar Clientes Ativos" — exige todos os dados (razão social, CNPJ, endereço completo, responsável, contato, etc.), só a frequência de visita fica de fora — v1.6
- ✓ Cliente importado por essa planilha entra direto no status "ganho", sem passar pelas 7 etapas do funil — v1.6
- ✓ Linha com dado obrigatório faltando na importação de ativos vira erro daquela linha só, nunca trava o lote — v1.6
- ✓ Duplicado é detectado na importação de ativos com a mesma lógica já usada nas outras importações — v1.6
- ✓ "Importar clientes" renomeada para "Importar Clientes em Prospecção" no menu — v1.6
- ✓ Planilha de prospecção exige só Nome Fantasia + Responsável; razão social e o resto ficam opcionais, preenchidos aos poucos — v1.6
- ✓ Cliente importado pela planilha de prospecção continua entrando em "Aguardando contato" — v1.6
- ✓ Ao tentar marcar um cliente como "ganho" faltando razão social ou endereço completo, o sistema bloqueia e orienta a completar a ficha antes — v1.6
- ✓ A trava de razão social/endereço no "ganho" é só de transição, nunca retroativa — clientes já "ganho" antes do v1.6 continuam funcionando normalmente — v1.6
- ✓ A caixinha de marcar "ganho" continua pedindo só CNPJ + frequência — não cresce para formulário maior — v1.6
- ✓ Ao definir frequência de visita semanal ou quinzenal de um cliente ativo, o vendedor também escolhe um dia fixo da semana — v1.6
- ✓ Ao definir frequência mensal, o vendedor escolhe a semana do mês (1ª/2ª/3ª/4ª/Última) + o dia da semana — v1.6
- ✓ Ao concluir uma visita, a sugestão de próxima data mira o dia fixo definido, nunca conta dias corridos a partir da conclusão — v1.6
- ✓ O sistema nunca sugere uma data de visita no passado — v1.6
- ✓ Vendedor vê na Agenda quais dos seus clientes ativos ainda não têm dia fixo definido (nunca teve frequência OU já tinha frequência mas falta o dia fixo) — v1.6
- ✓ "Importar CNPJ" e "Importar frequências" removidas do sistema por completo (código, rota, testes) — v1.6
...
- Editar cliente ativo já existente em massa (planilha de update) — a planilha de ativos só cria clientes novos, nunca atualiza; mesma decisão de v1.1 (importação nunca é upsert) — v1.6
- Planilha em massa para dia fixo/frequência — decisão explícita do dono; dia fixo se define individualmente, pela ficha do cliente ou pela Agenda — v1.6
- Semana do mês "5ª" (quinta ocorrência) na recorrência de visita — não existe na maioria dos meses; opções ficam 1ª/2ª/3ª/4ª/Última, mesmo padrão do Google Calendar/Outlook — v1.6
- Seed automático de primeira visita ao importar cliente ativo sem frequência — a primeira visita só é criada quando o vendedor definir a frequência/dia fixo depois — v1.6
- Caixinha de "ganho" crescer para formulário completo (razão social/endereço inline) — decisão explícita do dono: mantém a caixinha pequena, bloqueia com orientação para completar na ficha — v1.6
...
- **Estado do código pós-v1.6**: Next.js 16 (App Router) + Supabase (Postgres/Auth/RLS), sem backend Node separado. v1.3 adicionou: enum `frequencia_visita_enum` + 4 colunas nullable em `clientes` (nome_fantasia/cnpj/frequencia_pedidos/frequencia_visita), tabela `visitas`, coluna `resumo` em `tarefas`, RPC `agenda_do_vendedor()`, RPCs `concluir_tarefa_prospeccao`/`concluir_visita`, tabela `frequencias_pedido` (5ª lista editável), RPC `atualizar_frequencia_visita_lote`. v1.4 adicionou `mover_card_funil` com guard de CNPJ (7 parâmetros), `importar_clientes_lote` gravando `cnpj`/`nome_fantasia`, RPC `atualizar_cnpj_lote`. v1.5 adicionou `agenda_concluidos_do_vendedor(p_inicio, p_fim)`, tabela `motivos_conclusao_remota` (6ª lista editável) + FKs opcionais em `tarefas`/`visitas`, gatilhos de auditoria resolvendo FK de lista em texto legível. v1.6 adicionou 5 migrations (0024-0028): `clientes.razao_social` virou nullable; `mover_card_funil` ganhou dois guards novos de transição (razão social + endereço completo, 9 parâmetros no total contando os anteriores); `proxima_data_visita` reescrita para dia fixo (2 novos enums — dia da semana, semana do mês — + 2 colunas nullable em `clientes`, 4 parâmetros); nova RPC `importar_clientes_ativos_lote` (cria cliente já em "ganho", duplica os guards de razão social/CNPJ/endereço via função auxiliar `cliente_ativo_pronto_para_ganho` já que nunca passa por `mover_card_funil`); e `atualizar_cnpj_lote`/`atualizar_frequencia_visita_lote` foram removidas do banco (redundantes com a nova importação de ativos). O projeto continua com exatamente 4 exceções `SECURITY DEFINER` documentadas: `is_supervisor()`, `desativar_membro_equipe`/`reativar_membro_equipe`, `cidades_com_clientes_por_estado()` — nenhuma nova desde a v1.2, apesar de 15 migrations novas entre v1.3 e v1.6 (0013-0028).
- **Débito técnico conhecido**: nenhum item bloqueante. Um gap de verificação humana ficou formalmente aceito (Fase 8 — drag-and-drop com auto-scroll do kanban não foi exercitado por um teste automatizado nem por um drag real de mouse numa sessão anterior; risco julgado baixo por ser mudança isolada de CSS/layout, ver `.planning/phases/08-rolagem-por-coluna-no-kanban/08-VERIFICATION.md`). Um `eslint-disable` pontual e documentado permanece em `ClienteDetailSheet.tsx` (~linha 227) cobrindo um efeito de reset com ~13 setState. A caixinha antiga de "concluir tarefa" na ficha do cliente (não exige resumo, ao contrário do fluxo novo pela Agenda) continua coexistindo com o fluxo novo, aceito de propósito desde a Fase 16. **Novo do v1.6**: `importar_clientes_lote` liga produtos consumidos ao cliente recém-criado comparando razão social — quando a razão social é nula (planilha de prospecção, desde a Fase 26), essa ligação nunca casa; a Fase 26 compensou isso completando o vínculo na própria Server Action (por Nome Fantasia, nunca "chutando" um cliente em caso de ambiguidade), mas a correção definitiva (ligar produtos por posição da linha, não por razão social) ficou como recomendação de trabalho futuro, fora do escopo travado da fase. **Também do v1.6**: os testes automatizados de RLS que dependiam das contas seed `vendedor.a+test`/`vendedor.b+test` (apagadas durante a limpeza de dados de teste da preparação da planilha real) falham hoje com "Invalid login credentials" — não é regressão de código, mas precisa de decisão futura (recriar contas técnicas de teste, ou migrar os testes pra usar 2 dos 15 vendedores reais). **Verificação pendente**: o upload real de arquivo (não seed via script) nas telas "Importar Clientes Ativos" e "Importar Clientes em Prospecção" não foi exercitado ao vivo no navegador durante o v1.6 — limitação da ferramenta de automação de navegador desta sessão, não do código; recomendado testar na próxima oportunidade normal de uso.
...
| Trava de razão social/endereço no "ganho" é só de TRANSIÇÃO (3 condições E-adas: pediu ganho + status atual não é ganho + campo efetivo vazio), nunca de estado | Mesmo mecanismo do CNPJ-02 (v1.4) — é o que garante o grandfathering sem bloquear clientes antigos incompletos | ✓ Good |
| Importação de clientes ativos usa uma RPC nova (`importar_clientes_ativos_lote`) que duplica os guards de ganho verbatim, em vez de reusar/parametrizar `mover_card_funil` | A RPC de importação faz um INSERT direto já em "ganho", nunca passa pela RPC de mover card — duplicar o guard (com comentário cruzado) foi a única forma de não reabrir uma porta sem a trava | ✓ Good |
| `proxima_data_visita` avalia o caminho ANTIGO primeiro quando não há dia fixo definido, devolvendo o cálculo de offset fixo byte a byte — nunca uma segunda função paralela | Preserva o comportamento de todo cliente ativo pré-v1.6 (nenhum tinha dia fixo ainda) sem exigir migração de dado nenhuma; `tests/clientes/frequencia-visita.test.ts` continua servindo de oráculo de regressão sem edição | ✓ Good |
| Semana do mês limitada a 1ª/2ª/3ª/4ª/Última — nunca "5ª" | "5ª ocorrência" de um dia da semana não existe na maioria dos meses; mesmo padrão do Google Calendar/Outlook | ✓ Good |
| Definir o dia fixo pela primeira vez NUNCA recalcula retroativamente uma próxima visita já agendada — só rege a visita seguinte, após a atual ser concluída | Evita mudar de surpresa um compromisso que o vendedor já tinha combinado com o cliente | ✓ Good |
| Razão social em branco na importação de prospecção resolve para valor NULO, nunca texto vazio, em toda a cadeia até a gravação | Dois valores nulos nunca colidem numa constraint UNIQUE do Postgres; dois textos vazios colidem — resolver pra `""` faria a segunda linha sem razão social ser rejeitada como duplicata falsa | ✓ Good |
| Nome Fantasia vira a chave de reserva de duplicados quando razão social é nula (import de prospecção) | Sem uma segunda chave, nenhuma linha sem razão social seria comparável contra outra pro mesmo propósito de dedup | ✓ Good |
| Produtos consumidos de uma linha importada sem razão social são completados pela Server Action (por Nome Fantasia), nunca por uma segunda migration em `importar_clientes_lote` | Bug real descoberto no planejamento da Fase 26: a RPC liga produtos por razão social, e nulo nunca casa com nulo no Postgres — corrigir na RPC estava fora do escopo travado da fase; a correção definitiva (ligar por posição) ficou como trabalho futuro | ✓ Good |
| Nome exibido do cliente (card do funil + ficha) cai para Nome Fantasia quando razão social é nula, nunca mostra título em branco | Regressão visual criada pela própria v1.6 (razão social virou opcional) — `nomeExibicaoCliente()` é a autoridade única, mesmo padrão de `rotuloLocalizacao.ts` (Fase 23/quick m8q) | ✓ Good |
