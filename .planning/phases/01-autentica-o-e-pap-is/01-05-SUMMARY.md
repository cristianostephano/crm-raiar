...
## Open Item: Task 4 (Manual-Only, deferred)

**Status: NOT completed.** Per the project owner's explicit request, this plan is being closed out with Task 4 outstanding rather than blocking further work on it. Task 4 requires, when resumed:

1. Password-reset round-trip: request a reset link from `/forgot-password`, receive the real email, click through `/auth/confirm` to `/auth/reset-password`, set a new password, confirm login with it.
2. Invite-accept round-trip (closes AUTH-02): invite a real test address as Vendedor from "Gerenciar equipe," receive the real invite email, click through `/auth/confirm` to `/auth/reset-password`, set a password, confirm the new account logs in with a "Vendedor" role badge.

**AUTH-01 and AUTH-02 must not be recorded as fully manually-verified end-to-end anywhere until both round-trips above are confirmed and this checkpoint is explicitly approved.** REQUIREMENTS.md already shows both as "Complete" from prior plans (01-03/01-04) — that pre-existing state is left untouched by this plan (a known inconsistency, previously flagged in 01-03-SUMMARY.md), but no further "verified" claim is being added here.
