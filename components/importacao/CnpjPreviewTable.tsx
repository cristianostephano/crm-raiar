...
/** Só os dois rótulos que se aplicam a este fluxo — o terceiro valor
 * possível do vocabulário de status (visto no fluxo de importar clientes)
 * nunca ocorre aqui (não há coluna de Ação, não há decisão por linha). */
const STATUS_LABEL: Record<ValidatedRowCnpj["status"], string> = {
  ok: "OK",
  erro: "Erro",
}
...
/**
 * Step 3's paginated review table for the "CNPJ em massa" import (Fase 19,
 * IMP-03) — irmã estrutural de FrequenciaPreviewTable.tsx: mesmo PAGE_SIZE,
 * mesmo efeito de reinício de paginação, mesmo estado vazio, mesma barra de
 * paginação. A cor de borda por situação e a contagem de resumo vêm de
 * lib/importacao/preview.ts SEM MODIFICAÇÃO — este fluxo é um subconjunto
 * estrito ("ok"/"erro", nunca o terceiro valor do vocabulário) daquele
 * vocabulário de status, então a chave correspondente daquele mapa
 * simplesmente nunca é usada aqui.
 *
 * Sem propriedade de decisão por linha e sem função de mudança de decisão:
 * este fluxo não tem estado de decisão — por isso não existe coluna de
 * Ação. Ela não está escondida, ela não é montada.
 */
export function CnpjPreviewTable({
  linhas,
}: {
  ...
  // Uma nova rodada de validarLoteCnpj produz um array `linhas` novo —
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead>CNPJ a gravar</TableHead>
