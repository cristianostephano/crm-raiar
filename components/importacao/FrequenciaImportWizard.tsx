...
import {
  ...
  validarLoteFrequencia,
  type ConfirmarLoteFrequenciaResult,
  type ValidatedRowFrequencia,
} from "@/app/actions/importacaoFrequencia"
...
import { FrequenciaImportSummary } from "@/components/importacao/FrequenciaImportSummary"
import type { MappedRowFrequencia } from "@/lib/importacao/annotarLinhaFrequencia"
import {
  applyMapping,
...
import { SYSTEM_FIELDS_FREQUENCIA, type SystemFieldFrequencia } from "@/lib/importacao/typesFrequencia"
import type { ParsedFile } from "@/lib/importacao/types"
import { cn } from "@/lib/utils"

const MODELO_FILE_NAME = "modelo-frequencia-visita.xlsx"
...
const CONFIRM_ERROR =
  "Não foi possível concluir a atualização agora. Nenhuma frequência foi alterada. Tente novamente."
...
/**
 * Assistente de três passos da importação em massa de frequência de visita
 * (Fase 17, IMP-01, D1). Cópia estrutural de ImportWizard.tsx — mesma casca
 * de passos, mesma disciplina de estado (um estado por preocupação,
 * constantes de copy no topo, sinalizador de carga marcado de forma
 * síncrona antes de cada chamada assíncrona) — mas servindo a SEGUNDA lista
 * de campos (SYSTEM_FIELDS_FREQUENCIA) e chamando as ações de servidor deste
 * fluxo. Este fluxo não tem estado de decisão por linha (o vocabulário de
 * status é só "ok"/"erro", nunca "duplicado") — por isso o estado por-linha
 * do passo de revisão do assistente de clientes simplesmente não existe
 * aqui. ImportWizard.tsx NÃO é aberto para edição por este arquivo.
 */
export function FrequenciaImportWizard() {
  ...
  const [mapping, setMapping] = useState<ColumnMappingOf<SystemFieldFrequencia>>(
    {}
  )
  const [mappedRows, setMappedRows] = useState<MappedRowFrequencia[] | null>(
    null
  )

  // Passo 3 (revisão) — resultado de validarLoteFrequencia + seu próprio
  // estado de carga/erro.
  const [validating, setValidating] = useState(false)
  const [validatedRows, setValidatedRows] = useState<
    ValidatedRowFrequencia[] | null
  >(null)
  const [validationError, setValidationError] = useState<string | null>(null)

  // Passo de confirmação — estado de carga em voo/erro/resultado de
  // confirmarLoteFrequencia. Não existe estado de decisão por linha neste
  // fluxo (este vocabulário nunca tem "duplicado").
  const [confirming, setConfirming] = useState(false)
  const [confirmError, setConfirmError] = useState<string | null>(null)
  const [confirmResult, setConfirmResult] = useState<
    NonNullable<ConfirmarLoteFrequenciaResult["data"]> | null
  >(null)
  ...
  // síncrono de carga já usado no assistente de clientes.
  useEffect(() => {
    ...
    validarLoteFrequencia(mappedRows).then((result) => {
      setValidating(false)
      if (result.error) {
  ...
  function handleBaixarModelo() {
    const workbook = buildModeloFrequencia()
  ...
  function handleFileAccepted(acceptedFile: File) {
    ...
    parseArquivo(acceptedFile)
      .then((result) => {
        ...
        setParsed(result)
        // Sugestão automática de mapeamento por coluna calculada contra a
        // SEGUNDA lista de campos (SYSTEM_FIELDS_FREQUENCIA), não a de 14.
        const initialMapping: ColumnMappingOf<SystemFieldFrequencia> = {}
        result.headers.forEach((header, columnIndex) => {
          initialMapping[columnIndex] = suggestMapping(
            header,
  ...
  function handleMappingChange(
    columnIndex: number,
    target: MappingTargetOf<SystemFieldFrequencia>
  ) {
  ...
  function handleContinueFromMapping() {
    if (!parsed) return
    setMappedRows(
      applyMapping<SystemFieldFrequencia>(parsed.headers, parsed.rows, mapping)
    )
  ...
  function handleConfirmar() {
    ...
    // podem gravar o lote duas vezes (T-17-39).
    setConfirming(true)
    setConfirmError(null)

    confirmarLoteFrequencia(validatedRows).then((result) => {
      setConfirming(false)
      if (result.error) {
      ...
      {confirmResult !== null ? (
        ...
        <div className="flex flex-col gap-4">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            espera: a razão social (para encontrar o cliente) e a nova
            frequência de visita.
          </p>

          <ColumnMappingTable<SystemFieldFrequencia>
        ...
        <div className="flex flex-col gap-4">
          <h2 className="text-xl font-semibold">Revisar antes de atualizar</h2>
          ...
          {validatedRows ? (
            <FrequenciaPreviewTable linhas={validatedRows} />
          ) : null}
          ...
          <div className="flex gap-2">
            ...
            >
              {confirming ? "Atualizando frequências…" : "Confirmar atualização"}
            </Button>
          </div>
        </div>
      )}
