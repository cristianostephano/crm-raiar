 ...
 * (b) O atraso vem da autoridade única (`bucketDoItem`), com o `now`
 * recebido — este componente não decide atraso por conta própria.
 ...
 * nenhuma interação nova além das duas que a Lista já tem).
 */
 ...
 *
 * A COR da borda lateral é decidida em duas etapas: primeiro a cor de
 * origem (esmaecida para prospecção, primária para visita), depois — só se
 * atrasado — a cor de destaque de erro sobrescrevendo apenas essa cor. A
 * classe de atraso vem por ÚLTIMO na composição (`cn`, que faz merge de
 * utilitários conflitantes mantendo o último) de propósito: se a ordem
 * inverter, o item atrasado perde a informação de prospecção/visita (D-10).
 */
